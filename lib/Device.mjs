...
export default class Device {
  ...
  // MDC 0x25 — On Timer (Auto Power On)
  // Repeat: 0x01=Once, 0x02=Daily, 0x03=Mon-Fri, 0x04=Mon-Sat, 0x05=Sat-Sun
  async setOnTimer({
    displayId,
    enabled = true,
    repeat = 0x02,
    ...
    await this.sendCommand({
      displayId,
      commandId: 0x25,
      data: [enabled ? 0x01 : 0x00, repeat, hour, minute, 0x01],
    });
  }
  ...
  // MDC 0x10 — Display/model info. EM32DX returns a 3-byte code (e.g. 02 7a 01).
  async getModelInfo({
    displayId,
  } = {}) {
    try {
  ...
  // MDC 0xA7 — Real-time clock. EM32DX returns 7 bytes:
  //   [day, dayOfWeek(Sun=1), minute, month, yearHi, yearLo, 0x00]
  // Hour is not present in this register. day/month/year are verified;
  // dayOfWeek/minute are inferred from live observation.
  async getClock({
    displayId,
  } = {}) {
    try {
      ...
      if (r.length < 6) return { raw: Array.from(r) };
      return {
        day: r[0],
        dayOfWeek: r[1],   // 1 = Sunday ... 7 = Saturday (inferred)
        minute: r[2],      // inferred
        month: r[3],
        year: (r[4] << 8) | r[5],
        raw: Array.from(r),
